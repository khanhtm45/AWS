...
export function EditProductModal({ isOpen, onClose, onSubmit, productId }) {
  ...
  const getPresignedUrl = async (s3KeyOrUrl) => {
    ...
    try {
      const response = await fetch(
        `https://44.205.22.2:8080/api/s3/download-url?s3Key=${encodeURIComponent(s3KeyOrUrl)}`
      );
  ...
  const fetchProductData = async () => {
    ...
    try {
      const [productRes, variantsRes] = await Promise.all([
        fetch(`https://44.205.22.2:8080/api/products/${encodeURIComponent(productId)}`),
        fetch(`https://44.205.22.2:8080/api/products/${encodeURIComponent(productId)}/variants`)
      ]);
      ...
      if (productRes.ok) {
        ...
        if (productData.images && Array.isArray(productData.images) && productData.images.length > 0) {
          ...
          try {
            ...
            console.log(`   URL: https://44.205.22.2:8080/api/products/${encodeURIComponent(productId)}/media`);
            
            const mediaRes = await fetch(`https://44.205.22.2:8080/api/products/${encodeURIComponent(productId)}/media`);
              ...
              if (mediaData && mediaData.length > 0) {
                ...
                existingImages = await Promise.all(
                  sortedMedia.map(async (media, index) => {
                    ...
                    console.log(`   ➤ Fetch download URL: https://44.205.22.2:8080/api/s3/download-url?s3Key=${encodeURIComponent(media.s3Key)}&expirationMinutes=5`);
  ...
  const uploadSingleImageToS3 = async (file) => {
    try {
      ...
      console.log(`   URL: https://44.205.22.2:8080/api/s3/presigned-url`);
  ...
  const removeImage = async (imageId, mediaId, s3Key, isExisting) => {
    ...
    if (isExisting && mediaId) {
      ...
      try {
        ...
        const deleteUrl = `https://44.205.22.2:8080/api/products/${encodeURIComponent(productId)}/media/${encodeURIComponent(mediaId)}`;
  ...
  const handleUpdateProduct = async () => {
    try {
      ...
      const response = await fetch(`https://44.205.22.2:8080/api/products/${encodeURIComponent(productId)}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json'
        ...
        for (let i = 0; i < newImages.length; i++) {
          ...
          try {
            const mediaResponse = await fetch(
              `https://44.205.22.2:8080/api/products/${encodeURIComponent(productId)}/media`, 
              {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
    ...
    try {
      ...
      for (const variant of validVariants) {
        if (variant.isExisting) {
          ...
          await fetch(
            `https://44.205.22.2:8080/api/products/${encodeURIComponent(productId)}/variants/${encodeURIComponent(variant.variantId)}`,
            {
              method: 'PUT',
          ...
          await fetch(
            `https://44.205.22.2:8080/api/products/${encodeURIComponent(productId)}/variants`,
            {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify(variantPayload)

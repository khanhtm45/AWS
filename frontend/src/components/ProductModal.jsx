    ...
    if (imageToRemove && imageToRemove.uploadedToS3 && imageToRemove.s3Key) {
      try {
        ...
        const deleteResponse = await fetch(
          `${API_BASE_URL}/api/s3/delete?s3Key=${encodeURIComponent(imageToRemove.s3Key)}`,
          {
            method: 'DELETE'
          }
        );
  ...
  const handleCreateProduct = async () => {
    ...
    try {
      ...
      if (formData.images.length > 0) {
        ...
        try {
          for (let i = 0; i < formData.images.length; i++) {
            ...
            const mediaResponse = await fetch(`${API_BASE_URL}/api/products/${encodeURIComponent(productId)}/media`, {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json'
      ...
      if (error.message.includes('Failed to fetch') || error.message.includes('CORS')) {
        ...
        console.log('   1. Check if backend is running on', API_BASE_URL);
        console.log('   2. Verify CORS is configured properly in backend');
        console.log('   3. Try: curl -X POST', `${API_BASE_URL}/api/products`);
      }
    }
  };
  ...
  const handleCreateVariants = async () => {
    ...
    try {
      ...
      for (let i = 0; i < validVariants.length; i++) {
        ...
        console.log(`📍 API URL: ${API_BASE_URL}/api/products/${createdProductId}/variants`);
        
        // Call API to create variant
        const variantResponse = await fetch(`${API_BASE_URL}/api/products/${createdProductId}/variants`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
